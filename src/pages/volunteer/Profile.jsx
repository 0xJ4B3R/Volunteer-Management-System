...
import { User, Phone, CheckCircle2, Lock, Star, BadgeCheck, Eye, EyeOff, Globe, Hand, UserCheck, Hammer, HeartHandshake, ThumbsUp, Award, ShieldCheck, Users, Plus, X } from "lucide-react";
...
const getLevel = (hours, t) => {
  ...
  return { label: t("profile.badges.lordOfTheDeeds"), icon: <Icon />};
};
...
function Profile() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Translate skills and languages to current language
        const rawSkills = volunteerData.skills || [];
        const rawLanguages = volunteerData.languages || [];
        
        setSkills(translateSelectedItems(rawSkills, true));
        setLanguages(translateSelectedItems(rawLanguages, false));
  ...
  // Handle click outside skills and languages dropdowns to close them
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Check if click is outside skills dropdown
      const skillsDropdown = document.querySelector('.skills-section .options-dropdown');
      const skillsButton = document.querySelector('.skills-section .add-btn');
      if (skillsDropdown && skillsButton && 
          !skillsDropdown.contains(event.target) && 
          !skillsButton.contains(event.target)) {
        setShowSkillOptions(false);
      }

      // Check if click is outside languages dropdown
      const languagesDropdown = document.querySelector('.languages-section .options-dropdown');
      const languagesButton = document.querySelector('.languages-section .add-btn');
      if (languagesDropdown && languagesButton && 
          !languagesDropdown.contains(event.target) && 
          !languagesButton.contains(event.target)) {
        setShowLanguageOptions(false);
      }
    };

    if (showSkillOptions || showLanguageOptions) {
      document.addEventListener('mousedown', handleClickOutside);
    }
  ...
  }, [showSkillOptions, showLanguageOptions]);
  ...
  const getSkillMapping = () => {
    const englishSkills = t('skills', { returnObjects: true, lng: 'en' });
    const hebrewSkills = t('skills', { returnObjects: true, lng: 'he' });
    
    const englishToHebrew = {};
    const hebrewToEnglish = {};
    
    englishSkills.forEach((skill, index) => {
      const hebrewSkill = hebrewSkills[index];
      englishToHebrew[skill] = hebrewSkill;
      hebrewToEnglish[hebrewSkill] = skill;
    });
  ...
  const getLanguageMapping = () => {
    const englishLanguages = t('languages', { returnObjects: true, lng: 'en' });
    const hebrewLanguages = t('languages', { returnObjects: true, lng: 'he' });
    
    const englishToHebrew = {};
    const hebrewToEnglish = {};
    
    englishLanguages.forEach((language, index) => {
      const hebrewLanguage = hebrewLanguages[index];
      englishToHebrew[language] = hebrewLanguage;
      hebrewToEnglish[hebrewLanguage] = language;
    });
  ...
  const isValueInArray = (value, array, isRTL) => {
    ...
    if (isRTL) {
      const { hebrewToEnglish } = getSkillMapping();
      const { hebrewToEnglish: langMapping } = getLanguageMapping();
      const englishValue = hebrewToEnglish[value] || langMapping[value];
      if (englishValue && array.includes(englishValue)) return true;
    } else {
      // If LTR (English), check if the Hebrew equivalent is in the array
      const { englishToHebrew } = getSkillMapping();
      const { englishToHebrew: langMapping } = getLanguageMapping();
      const hebrewValue = englishToHebrew[value] || langMapping[value];
      if (hebrewValue && array.includes(hebrewValue)) return true;
    }
    
    return false;
  };

  const addValueToArray = (value, array, isRTL) => {
  ...
  const removeValueFromArray = (value, array, isRTL) => {
    if (!array || array.length === 0) return array;
    return array.filter(item => item !== value);
  };

  // Translate selected items to current language
  const translateSelectedItems = (items, isSkill = true) => {
    if (!items || items.length === 0) return [];
    
    const mapping = isSkill ? getSkillMapping() : getLanguageMapping();
    const { englishToHebrew, hebrewToEnglish } = mapping;
    
    return items.map(item => {
      // If current language is Hebrew, translate from English to Hebrew
      if (i18n.language === 'he') {
        return englishToHebrew[item] || item;
      } else {
        // If current language is English, translate from Hebrew to English
        return hebrewToEnglish[item] || item;
      }
    });
  };

  // Convert selected items to English for database storage
  const convertToEnglishForStorage = (items, isSkill = true) => {
    if (!items || items.length === 0) return [];
    
    const mapping = isSkill ? getSkillMapping() : getLanguageMapping();
    const { hebrewToEnglish } = mapping;
  ...
  const handleAddSkill = async (skill) => {
    // Convert the skill to English to check against stored data
    const { hebrewToEnglish } = getSkillMapping();
    const englishSkill = hebrewToEnglish[skill] || skill;
    
    // Check if already exists in current skills (convert all to English for comparison)
    const currentEnglishSkills = convertToEnglishForStorage(skills, true);
    if (currentEnglishSkills.includes(englishSkill)) return;
    ...
    try {
      if (volunteerId) {
        ...
        const englishSkills = convertToEnglishForStorage(newSkills, true);
        ...
        toast({ 
          ...
          description: t("profile.toast.addedDesc", { skill })
        });
      }
    } catch (error) {
      // Revert on error
  ...
  const handleAddLanguage = async (language) => {
    // Convert the language to English to check against stored data
    const { hebrewToEnglish } = getLanguageMapping();
    const englishLanguage = hebrewToEnglish[language] || language;
    
    // Check if already exists in current languages (convert all to English for comparison)
    const currentEnglishLanguages = convertToEnglishForStorage(languages, false);
    if (currentEnglishLanguages.includes(englishLanguage)) return;
    ...
    try {
      if (volunteerId) {
        const volunteerRef = doc(db, "volunteers", volunteerId);
        // Convert to English for storage
        const englishLanguages = convertToEnglishForStorage(newLanguages, false);
        await updateDoc(volunteerRef, {
        ...
        toast({ 
          title: t("profile.toast.addedTitle"), 
          description: t("profile.toast.languageAdded", { language })
        });
      }
    } catch (error) {
      // Revert on error
  ...
  const getPasswordError = (password) => {
    if (!password || password.trim().length === 0) {
      return t("profile.errors.fillAllFields");
    }

    if (password.length < 8) {
      return t("profile.errors.passwordTooShort");
    }

    if (!/[A-Z]/.test(password)) {
      return t("profile.errors.passwordNoUppercase");
    }

    if (!/[a-z]/.test(password)) {
      return t("profile.errors.passwordNoLowercase");
    }

    if (!/[0-9]/.test(password)) {
      return t("profile.errors.passwordNoNumber");
    }

    if (!/[^A-Za-z0-9]/.test(password)) {
      return t("profile.errors.passwordNoSpecialChar");
    }
  ...
  const handlePasswordChange = async () => {
    // Validate all fields
    const currentPasswordError = !currentPassword ? t("profile.errors.fillAllFields") : null;
    const newPasswordError = getPasswordError(newPassword);
    const confirmPasswordError = newPassword !== confirmPassword ? t("profile.errors.passwordMismatch") : null;

    if (currentPasswordError || newPasswordError || confirmPasswordError) {
      setPasswordChangeStatus({ 
        type: "error", 
        message: currentPasswordError || newPasswordError || confirmPasswordError 
      });
      return;
    }
    ...
    try {
      setPasswordChangeStatus({ type: "loading", message: t("profile.changingPassword") });
      ...
      if (isPlainTextPassword) {
        ...
        try {
          ...
          setPasswordChangeStatus({ 
            type: "error", 
            message: t("profile.errors.passwordVerificationFailed") 
          });
          return;
        }
      }
      ...
      try {
        ...
        setPasswordChangeStatus({ type: "error", message: t("profile.errors.passwordCreationFailed") });
        return;
      }
      ...
      if (error.code === 'permission-denied') {
        errorMessage = t("profile.errors.permissionDenied");
      } else if (error.code === 'network-request-failed') {
        errorMessage = t("profile.errors.networkError");
      }
          ...
          {showLangOptions && (
            <div className={`lang-options ${i18n.language === 'he' ? 'rtl-popup' : 'ltr-popup'}`}>
              <button onClick={async () => {
                ...
                // Translate selected skills and languages when language changes
                if (skills.length > 0) {
                  setSkills(translateSelectedItems(skills, true));
                }
                if (languages.length > 0) {
                  setLanguages(translateSelectedItems(languages, false));
                }
              }}>
                English
              </button>
          ...
          {activeTab === "profile" && (
            <div className="profile-overview">
                ...
                <div className="profile-hours">
                  <div className="hours-progress-container">
                    ...
                    <div className="hours-display">
                      <span className="hours-number">{userProfile.totalHours.toFixed(2).replace(/\.?0+$/, '')}</span>
              ...
              <div className="profile-stats-grid">
                <div className="stat-card">
                  <div className="stat-icon"><User size={36} /></div>
                    <p className="stat-label">{t("profile.username")}<span className="mobile-colon">:</span></p>
                ...
                <div className="stat-card">
                  <div className="stat-icon badge-icon-container">{level.icon}</div>
                    <p className="stat-label">{t("profile.status")}<span className="mobile-colon">:</span></p>
                  <p className="stat-value">{level.label}</p>
                </div>
                <div className="stat-card">
                  <div className="stat-icon"><Phone size={36} /></div>
                    <p className="stat-label">{t("profile.phone")}<span className="mobile-colon">:</span></p>
                  <p className="stat-value"><span dir="ltr">{userProfile.phone}</span></p>
                </div>
                <div className="stat-card">
                  <div className="stat-icon"><CheckCircle2 size={36} /></div>
                    <p className="stat-label">{t("profile.completedSessions")}<span className="mobile-colon">:</span></p>
              ...
              <div className="skills-section">
                <h4 className="section-title">{t("profile.skills")} ({skills.length})</h4>
              ...
              <div className="languages-section">
                <h4 className="section-title">{t("profile.languages")} ({languages.length})</h4>
                ...
                {showLanguageOptions && (
                  <div className="options-dropdown">
                    <div className="options-list">
                      {(t('languages', { returnObjects: true }))
