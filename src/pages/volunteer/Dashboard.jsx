...
const Dashboard = () => {
  ...
  const getLevel = (hours) => {
    ...
    if (hours >= 200 && hours < 420)
      return { label: t("dashboard.levels.Humanitarian"), icon: <Globe size={38} />, nextLevel: null, hoursToNext: 0 };
    return { label: t("dashboard.levels.Lord of the deeds"), icon: <Zap size={38} />, nextLevel: null, hoursToNext: 0 };
  };
        ...
        if (data.status === "inProgress") {
          ...
          if (data.volunteers && Array.isArray(data.volunteers)) {
            ...
            if (userVolunteer) {
              ...
              // Only show sessions that haven't ended yet
              if (sessionEndDateTime > now) {
                const startTime = data.startTime || t('dashboard.checkIn.subtitle');
                const endTimeStr = data.endTime || "";
                const timeRange = endTimeStr ? `${startTime} - ${endTimeStr}` : startTime;
                
                // Translate session category
                const translatedCategory = translateSessionCategory(data.customLabel || data.category);
  ...
  const fetchRecentActivity = async () => {
    try {
      ...
      snapshot.forEach((doc) => {
        ...
        if (data.confirmedAt) {
          try {
            ...
            if (diffDays === 0) {
              timeAgo = t('dashboard.time.today');
            } else if (diffDays === 1) {
        ...
        if (status === "present") {
          ...
          icon = AlertCircle;
          iconColor = "dash-icon-red";
        } else if (status === "cancelledByVolunteer") {
      ...
      // Fallback activity data
      setRecentActivity([
        { id: 'fallback-1', type: 'present', text: t('dashboard.activity.present', { notes: '' }), time: t('dashboard.time.daysAgo', { count: 2 }), icon: CheckCircle2, iconColor: 'dash-icon-green' }
      ]);
    }
  };
                ...
                {recentActivity.map((activity) => {
                  ...
                  const text = (() => {
                    switch (type) {
                      ...
                      case 'absent':
                        return t('dashboard.activity.absent', { notes });
                      case 'cancelledByVolunteer':
                        return t('dashboard.activity.cancelledByVolunteer');
                      default:
                        return t('dashboard.activity.generic', { status: type, notes });
                    }
                  })();
                  ...
                  const time = (() => {
                    ...
                    if (count < 14) {
                      return t('dashboard.time.weekAgo');
                    }
                    const weeks = Math.floor(count / 7);
                    return t('dashboard.time.weeksAgo', { count: weeks });
                  })();
